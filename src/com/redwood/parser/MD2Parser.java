...
import com.redwood.animation.KeyFrame;
...
public class MD2Parser extends AParser implements IParser {
	...
	public AnimationObject3d getParsedAnimationObject() {
		...
		Log.d(Kitty3d.TAG, "Creating object " + co.name);
	...
	public void parse() {
		...
		Log.d(Kitty3d.TAG, "Start parsing MD2 file");
		try {
			header.parse(stream);
	...
	private void getFrames(BufferedInputStream stream, byte[] bytes)
		...
		for (int i = 0; i < header.numFrames; i++) {
			...
			Log.d(Kitty3d.TAG, "frame name: " + name);
