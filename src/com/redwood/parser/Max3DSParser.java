...
public class Max3DSParser extends AParser implements IParser {
	...
	public void parse() {
		...
		Log.d(Kitty3d.TAG, "Start parsing object");
		...
		try {
			...
			{
				Log.d(Kitty3d.TAG, "Not a valid .3DS file!");
				return;
			}
			else
			{
				Log.d(Kitty3d.TAG, "Found a valid .3DS file");
			}
			while(!endReached)
			{
				readChunk(stream);
		...
		Log.d(Kitty3d.TAG, "End parsing object");
	}
	...
	private void readChunk(InputStream stream) throws IOException {
		...
		case OBJECT_BLOCK:
			...
			Log.d(Kitty3d.TAG, "Parsing object " + currentObjName);
			break;
		case TRIMESH:
			if(firstObject)
			{
	...
	public Object3dContainer getParsedObject() {
		Log.d(Kitty3d.TAG, "Start object creation");
		...
		for (int i = 0; i < numObjects; i++) {
			...
			Log.d(Kitty3d.TAG, "Creating object " + o.name);
		...
		Log.d(Kitty3d.TAG, "Object creation finished");
