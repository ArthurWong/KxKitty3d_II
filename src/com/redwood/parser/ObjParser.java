...
public class ObjParser extends AParser implements IParser {
	...
	public void parse() {
		...
		Log.d(Kitty3d.TAG, "Start parsing object " + resourceID);
		Log.d(Kitty3d.TAG, "Start time " + startTime);
		...
		try {
			while ((line = buffer.readLine()) != null) {
				...
				if (type.equals(VERTEX)) {
					...
					if(firstObject)
					{
						Log.d(Kitty3d.TAG, "Create object " + objName);
						co.name = objName;
						firstObject = false;
					}
					else
		...
		Log.d(Kitty3d.TAG, "End time " + (endTime - startTime));
	}
